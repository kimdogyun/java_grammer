...
//        예외처리 목적 : 1)사용자에게 적절한 에러 메시지 전달 2)예외발생시 디버깅을 위한 로그 기록
...
//            Exception은 모든 예외클래스의 조상클래스. 구체적인 예외클래스가 catch부에 있다면, 해당 부분으로 우선 분기처리
            ...
            System.out.println("예외가 발생하든 발생하지 않든 무조건 실행 되는 구문");
        }
//        예외처리를 하지 않을 경우 프로그랜은 강제종료, 예외처리를 할 경우 프로그램은 예외가 발생하더라도 지속
